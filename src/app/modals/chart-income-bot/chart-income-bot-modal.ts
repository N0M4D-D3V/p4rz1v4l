...
export class ChartIncomeBotModalComponent implements OnInit, OnDestroy {
  ...
  private getResultsInfoSubscription() {
    this.resultsSubscriptionBacktest = this.chartIncomeBotService.resultsInfo$.subscribe((data) => {
      this.resultsInfo = data;
    });
  }

  private getResultsDataSubscription() {
    this.resultsSubscriptionBacktest = this.chartIncomeBotService.resultsData$.subscribe((data) => {
