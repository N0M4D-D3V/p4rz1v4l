...
export class ChartExecuteBotComponent implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (
      changes["resultsInfoIncome"] &&
      changes["resultsInfoIncome"].currentValue
    ) {
      const closeValues = this.resultsDataIncome.map((item: any) => item.close);
      const monthLabels = this.resultsDataIncome.map((item: any) =>
        this.getMonthNameFromTimestamp(item.timestamp)
      );
